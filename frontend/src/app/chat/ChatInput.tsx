...
export default function ChatInput({ onSubmit, onStop, loading }: { onSubmit: Function, onStop: Function, loading: boolean }) {
    ...
    function handleSubmit(e: React.InputEvent) {
        e.preventDefault()
        if (text.trim() === "") return
        onSubmit(text)
        setText("")
    ...
    function handleStop(e: React.InputEvent) {
        e.preventDefault()
        onStop()
    }
    ...
    return (
        <Form onSubmit={handleSubmit} action="" className=" bg-amber-50 rounded w-[65%] flex flex-col m-2 mb-0" >
            <textarea onKeyDown={handleKeyDown} value={text} onChange={(e) => setText(e.target.value)} className="input-text  text-gray-800 text-wrap outline-0 p-2 m-2 resize-none" />
